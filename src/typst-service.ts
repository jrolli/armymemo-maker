...
/**
 * Derive the download filename from the memo's SUBJECT line (design D1/D3 of
 * add-subject-download-filename). The vendored armymemo package emits no
 * subject metadata, so this reads the first `subject: "..."` string literal
 * from the compiled source snapshot; anything else (expression subjects,
 * missing subject) falls back to `memo.pdf`.
 */
